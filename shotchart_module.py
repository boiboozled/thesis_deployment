...
def shot_chart(data,x, y, kind="scatter", title="", color="b", cmap=None,
    ...
    if kind == "scatter":
        ...
        sns.kdeplot(data,x=x, y=y, fill=kde_shade, cmap=cmap, ax=ax, **kwargs)
        ax.set_xlabel('')
        ax.set_ylabel('')
...
def convert_seaborn_to_img(fig):
    ...
    plt.close()
    s = base64.b64encode(s.getbuffer()).decode("utf-8")
    return f'data:image/png;base64,{s}'

def shot_chart_plotly(data, x,y,kind="scatter",fig=None, **kwargs):
    if not fig:
        fig = go.Figure()
